...
OUT_DIR='/data/data_repo/castrocp/SSC_pipeline/phase3-2/SplitPhases/Families_151-226'
REFERENCE='/nfs/boylelabnr_turbo/genomes/hg38/GRCh38_full_analysis_set_plus_decoy_hla.fa'
SAMPLE_FILE='/data/data_repo/castrocp/SSC_pipeline/ID_mapping/ssc_phase3-2_quads_151-226_sample_ids.txt'
...
do
	/usr/bin/time -v java -jar $EBROOTGATK/GenomeAnalysisTK.jar -T SelectVariants -R $REFERENCE -V $file -sf $SAMPLE_FILE -o $OUT_DIR/$(basename $file .vcf.gz).families151-226.vcf.gz |& tee $OUT_DIR/$(basename $file .vcf.gz).families151-226.log
