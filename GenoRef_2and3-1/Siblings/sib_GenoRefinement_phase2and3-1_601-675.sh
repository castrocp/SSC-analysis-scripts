...
PHASE_DIR='/data/data_repo/castrocp/SSC_pipeline/phase2and3-1/SplitPhase/Fams601-675/'
OUT_DIR='/data/data_repo/castrocp/SSC_pipeline/Siblings/phase2and3-1_sibs/GenoRefinement/Fams601-675/'
...
for file in $PHASE_DIR*chr1.recalibrated_variants.fams601-675.vcf.gz
...
do
	...
	/usr/bin/time -v java -Xmx8G -jar $EBROOTGATK/GenomeAnalysisTK.jar -R $REF_FILE -T CalculateGenotypePosteriors --supporting $SUPPORTING -ped $PED -V:VCF $file -o $OUT_DIR$base.CGP_ped.vcf.gz -pedValidationType SILENT |& tee $OUT_DIR$base.CGP_ped.log
	
	# Tag genotypes with GQ<20 or DP<10
	# Variants aren't removed, only tagged with "low"
	/usr/bin/time -v java -Xmx8G -jar $EBROOTGATK/GenomeAnalysisTK.jar -T VariantFiltration -R $REF_FILE -V:VCF $OUT_DIR$base.CGP_ped.vcf.gz -G_filter "GQ < 20.0 || DP < 10.0" -G_filterName low -o $OUT_DIR$base.CGP_ped.tag_lowGQ_DP.vcf.gz |& tee $OUT_DIR$base.CGP_ped.tag_lowGQ_DP.log
