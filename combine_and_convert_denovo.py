...
This version of the script is meant to be run on a phase that hasn't been further split up. phase2and3-1, for example, has been split
into smaller batches, and a separate script should be used for that. 
...
Run as: 
python combine_and_convert_denovo.py phase4 proband

That will process the probands from families from phase4
"""
...
try:
    ...
    print("Enter the phase to be processed after the script name")
...
subprocess.run(['mkdir', PHASE_DIR + '/Candidate_DeNovo'])
CAND_DENOVO_OUT_DIR = PHASE_DIR + '/Candidate_DeNovo' 
